//seleccion niveles completando el primer nivel
...
export default class scene11 extends Phaser.Scene
{
    ...
    preload(){
		...
		this.load.image('flecha', 'assets/Flecha_Volver.png')
	}
	create()
	{
		...
		.on('pointerdown', () => this.scene.start('scene10') && click.play(),);
		var bloqueado = this.add.image(680, 290, 'nivel1').setScale(0.6).setInteractive().on('pointerdown', () => this.scene.start('nivel2') && click.play(),);
        var bloqueado = this.add.image(968, 290, 'bloqueado').setScale(0.6)
